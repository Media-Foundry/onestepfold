...
mkdir -p "$OUTPUT_ROOT/meta" "$OUTPUT_ROOT/logs"
{
...
PY
} > "$OUTPUT_ROOT/meta/environment.txt" 2>&1
...
for setting in "c1_s1 1 1" "c1_s2 1 2" "c1_s5 1 5" \
               "c2_s1 2 1" "c2_s2 2 2" "c2_s5 2 5" \
               "c4_s1 4 1" "c4_s2 4 2" "c4_s5 4 5"; do
  read -r name cycles steps <<< "$setting"
  run_one "$INPUT_JSON" "$SEED" "$name" "$cycles" "$steps" "$OUTPUT_ROOT/$name/seed-$SEED"
done
...
cat > "$OUTPUT_ROOT/meta/timing_components.json" <<'EOF'
{
  "sequence_ms": null,
  "trunk_ms": null,
  "structure_ms": null,
